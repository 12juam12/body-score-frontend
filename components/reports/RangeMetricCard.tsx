  ...
  return (
    <div className="rounded-2xl border border-border bg-surface p-6 shadow-sm">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3 text-center">
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted">{metric.actualLabel ?? "Actual"}</p>
          <p className="mt-0.5 text-lg font-semibold text-foreground">
            {metric.actualValue} <span className="text-xs font-normal text-muted">{metric.unit}</span>
          </p>
        </div>
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted">{metric.rangeLabel ?? "Rango esperado"}</p>
          <p className="mt-0.5 text-lg font-semibold text-foreground">
            {metric.minValue === metric.maxValue ? metric.minValue : `${metric.minValue}–${metric.maxValue}`}{" "}
            <span className="text-xs font-normal text-muted">{metric.unit}</span>
          </p>
        </div>
        <div>
          <p className="text-[11px] uppercase tracking-wide text-muted">{metric.differenceStatLabel ?? "Diferencia"}</p>
