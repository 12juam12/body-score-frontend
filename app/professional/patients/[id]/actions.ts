...
export async function reviewOwnPatient(
...
export async function editOwnPatient(_prevState: EditFormState, formData: FormData): Promise<EditFormState> {
  const patientId = formData.get("patientId");
  ...
  try {
    const response = await apiFetch(`/api/professionals/me/patients/${patientId}`, {
      method: "PUT",
      body: JSON.stringify({
        firstName,
        lastName,
