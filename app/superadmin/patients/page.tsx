...
import type { PageResponse, PatientListItem } from "@/lib/types";
...
export default async function PatientsListPage({ searchParams }: PatientsListPageProps) {
  ...
  const result = await apiJson<PageResponse<PatientListItem>>(`/api/patients?${params.toString()}`);
  ...
  return (
    <div className="flex flex-col gap-6">
        ...
        <tbody>
          {result.items.map((patient) => (
            <tr key={patient.patientId} className="border-b hover:bg-zinc-50 dark:hover:bg-zinc-900">
              <td className="py-2">
                <Link href={`/superadmin/patients/${patient.patientId}`} className="underline">
      ...
      <div className="flex gap-2 text-sm">
        {currentPage > 0 ? (
          <Link
            href={`/superadmin/patients?${new URLSearchParams({ ...(status && { status }), ...(search && { search }), page: String(currentPage - 1) }).toString()}`}
            className="underline"
          >
            Anterior
          </Link>
